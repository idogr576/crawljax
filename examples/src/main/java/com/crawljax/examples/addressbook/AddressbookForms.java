...
public class AddressbookForms {
	...
	public static void newGroup(InputSpecification inputAddressBook) {
		...
		FormInput username = newGroupForm.inputField(InputType.TEXT, new Identification(How.name, "email"));
		username.inputValues("tsigalko18@gmail.com");

		FormInput password = newGroupForm.inputField(InputType.TEXT, new Identification(How.name, "password"));
		password.inputValues("123456");

		inputAddressBook.setValuesInForm(newGroupForm).beforeClickElement("input").withAttribute("value", "Sign-Up");
	}
