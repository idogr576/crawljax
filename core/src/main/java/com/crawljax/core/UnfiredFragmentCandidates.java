...
//	private StateVertex nextBestState = null;
	...
	@Inject UnfiredFragmentCandidates(BrowserConfiguration config, Provider<StateFlowGraph> sfg,
			MetricRegistry registry, CrawlRules crawlRules) {
		this.sfg = sfg;
...
//					StateVertex bestState = fragmentManager.getClosestUnexploredFragment(state, stateMachine.getOnURLSet(), statesWithCandidates);
					if(unexploredStates && !afterBacktrack) {
						try{
							bestState = fragmentManager.getClosestUnexploredState(state, stateMachine.getOnURLSet(), statesWithCandidates, applyNonSelAdvantage);
						}catch(Exception ex) {
...
//						}
					...
					if(bestState == null) {
						...
						bestAction = null;
						LOG.info("No unexplored elements available!!. So switching to best state: " + bestState.getName());
					}
				}
				else {
					bestState = state;
...
//					bestAction = queue.get(queue.size()-1);
	...
	void rediscoveredState(StateVertex state) {
		...
		if(restoreConnectedEdges ) {
			...
			for(StateVertex connectedState: connectedStates) {
				LOG.info("Restoring connected {} because {} is rediscovered", connectedState.getName(), state.getName());
	 ...
	 */
	public StateVertex awaitNewTask(StateVertex currentState, List<StateVertex> onURLSet, FragmentManager fragmentManager) throws InterruptedException {
		...
		try {
			next =	fragmentManager.getClosestUnexploredState(currentState, onURLSet, statesWithCandidates, applyNonSelAdvantage);
		}catch(Exception ex) {
    ...
    public void mapInput(Eventable event, List<FormInput> worked) {
        ...
        LOG.info("input for {}, {}",event.getId(), this.inputMap.get(event.getId()).size());
    }
