...
public class Crawler {
	...
	private Eventable getResetEquivalent() {
		...
		try {
			if(stateMachine.getStateFlowGraph().canGoTo(stateMachine.getCurrentState(), stateMachine.getStateFlowGraph().getInitialState())) {
				Eventable returnEvent= stateMachine.getStateFlowGraph().getShortestPath(stateMachine.getCurrentState(), stateMachine.getStateFlowGraph().getInitialState()).get(0);
				LOG.error("Found direct edge between current {} and index :  {} ", stateMachine.getCurrentState().getName(), returnEvent.getId());
				return returnEvent;
			}
		}catch(Exception ex) {
		...
		ImmutableSet<Eventable> toReachIndex = stateMachine.getStateFlowGraph().getIncomingClickable(stateMachine.getStateFlowGraph().getInitialState());
		
		for(Eventable toReach: toReachIndex ) {
			try {
			CandidateElement elementToReach = toReach.getSourceStateVertex().getCandidateElement(toReach.getElement().getNode()).get(0);
			CandidateElement equivalentCandidate = stateMachine.getCurrentState().getCandidateElement(toReach);
			if(equivalentCandidate != null && fragmentManager.areRelated(elementToReach.getClosestFragment(), equivalentCandidate.getClosestFragment())) {
				LOG.info("Found equivalent edge from {}. Using it on {}", toReach.getSourceStateVertex().getName(), stateMachine.getCurrentState().getName());
	...
	public void reset(int nextTarget) {
		...
		if(UseEquivalentReset) {
			if(stateMachine!= null && stateMachine.getCurrentState().equals(stateMachine.getStateFlowGraph().getInitialState())) {
				equivalentResetDone = true;
			}
			else {
				Eventable equivalentEvent = getResetEquivalent();
				if(equivalentEvent!= null && fireEventWithInputs(equivalentEvent, equivalentEvent.getRelatedFormInputs())) {
					...
					if(stateMachine.getCurrentState().equals(stateMachine.getStateFlowGraph().getInitialState())){
		...
		if (path != null) {
			try {
				...
				if(!crawlTask.equals(stateMachine.getCurrentState())) {
					...
					LOG.info("Tried reaching {} but Reached a near duplicate {}", crawlTask.getName(), stateMachine.getCurrentState().getName());
...
//					stateMachine.setCurrentState(currentOnURLState);
	...
	private ArrayList<Eventable> followOld(CrawlPath path, StateVertex targetState)
...
//			}
	...
	private ArrayList<Eventable> followNew(CrawlPath path, StateVertex targetState)
			throws CrawljaxException {
		...
		StateVertex currState = null;
		if (stateMachine != null) {
			LOG.info("Backtracking start from {}", stateMachine.getCurrentState().getName());
			currState = stateMachine.getCurrentState();
		}
		if (currState == null) {
			LOG.info("Backtracking Error!! starting from {}", context.getSession().getInitialState().getName());
		...
		for (Eventable clickable : path) {
			...
			if(!clickable.getSourceStateVertex().equals(currState)) {
				...
				if(avoidDifferentBacktracking && fragmentManager.cacheStateComparision(clickable.getSourceStateVertex(), currState, true)==StateComparision.DIFFERENT) {
					...
					throw new StateUnreachableException(clickable.getSourceStateVertex(),
							String.format("The path didn't result in the desired %1$ but in %2$", clickable.getSourceStateVertex(), currState)); 
				}
...
//					LOG.info("BackTracking exploration {}", clone.getId());
				...
				if(newCandidate!=null) {
					...
					if(!fragmentManager.areRelated(newCandidate.getClosestFragment(), oldCandidate.getClosestFragment())){
						LOG.info("{} {} and {} {} are unrelated", newCandidate.getClosestFragment().getId(),
								currState.getName(), oldCandidate.getClosestFragment().getId(), 
								clickable.getSourceStateVertex().getName());
						if(avoidUnrelatedBacktracking ) {
							LOG.info("Avoiding unrelated backtracking!!");
							throw new StateUnreachableException(targetState, "Avoiding backtracking through unrelated fragments");
						}
						else {
					...
					if(!newCandidate.isDirectAccess()) {
						...
						LOG.info("Backtracking event {} explored already!! for equivalent candidate {}", clone.getId(), newCandidate.getIdentification().getValue());
					}
				}
				else {
					LOG.error("Could not find equivalent element in backtracking {} for candidate  {}", clone.getId(), clone.getIdentification().getValue());
				}
...
//			}
			...
			if(!clickable.getTargetStateVertex().equals(stateMachine.getCurrentState())) {
				...
				LOG.info("Backtracking state has changed from {} to {} ", clickable.getTargetStateVertex().getName(), stateMachine.getCurrentState());
				...
				if(clickable.getSourceStateVertex().equals(currState)) {
					...
					LOG.info(" Removing expired Edge {} to {}", clickable.getId(), clickable.getTargetStateVertex().getName());
					graphProvider.get().removeEdge(clickable);
					//TODO: should I remove the vertex as well? 
					//TODO: APTED comparison may not be enough to remove the vertex. May be make sure before removing state.
				}
				...
				if(stateMachine.getCurrentState() instanceof HybridStateVertexImpl) {
					...
					boolean assignDynamic = true;
					StateComparision comp = fragmentManager.cacheStateComparision(newState, expectedState, assignDynamic);

					LOG.info("changed backtracking state {} and  {} are {}", newState.getName(), expectedState.getName(), comp);
					StateComparision prevComp = fragmentManager.cacheStateComparision(clickable.getSourceStateVertex(), currState, true);
					if(avoidDifferentBacktracking  && comp == StateComparision.DIFFERENT &&  prevComp != StateComparision.DIFFERENT) {
						...
						throw new StateUnreachableException(expectedState,
								String.format("The path didn't result in the desired %1$ but in %2$", expectedState, newState)); 
					}
				}
				
				
				LOG.info("Finding shortest path from {} to {}", stateMachine.getCurrentState().getName(), targetState.getName());
				CrawlPath newPath = null;
				try {
				newPath = CrawlPath.copyOf(shortestPathTo(targetState, stateMachine.getCurrentState()), targetState.getId());
				}catch(Exception ex) {
				...
				if(newPath!=null) {
					...
					if(newPathLength < oldPathLength) {
						LOG.info("Found shorter path from chngwd bt state {} to {}", stateMachine.getCurrentState().getName(), targetState.getName());
				...
				LOG.debug("Reached the intended backtracking state {} using {}", stateMachine.getCurrentState().getName(), clone.getId());
			}
			currState = stateMachine.getCurrentState();
...
//					avoidPath(crawlpath);
				...
				throw new StateUnreachableException(targetState,
						String.format("The path didn't result in the desired %1$ but in %2$", targetState.getName(), currState.getName()));
			}
	...
	private List<FormInput> getInputElements(Eventable eventable){
		...
		LOG.info("Changing related inputs for {} with {}  to {} ",eventable.getId(), formInputsExisting.size(), formInputs.size());
		return formInputs;
	}
	...
	private List<FormInput> orderFormInputs(List<FormInput> formInputs) {
		...
		for(FormInput input: formInputs) {
			try {
				Node inputNode= XPathHelper.getBelongingNode(input, stateMachine.getCurrentState().getDocument());
				if(inputNode!=null) {
...
//		waitForRefreshTagIfAny(event);
	...
	private boolean fireEvent(Eventable eventable, boolean resolveXpath) {
		...
		if (eventable.getIdentification().getHow().toString().equals("xpath")
				&& eventable.getRelatedFrame().equals("") && resolveXpath) {
		...
		while (action != null && !interrupted) {
			...
			if (element.allConditionsSatisfied(browser)) {
				...
				if(element.wasExplored()) {
					...
					LOG.info("Duplicate access for {} \n seed {}", element.getIdentification().getValue(), eventableId);
				}
				...
				if (fired) {
					...
					if (action != null) {
						...
						if(added) {
							...
							actions.add(action);
							candidateActionCache.addActions(actions,
								stateMachine.getCurrentState());
							}
					}
				}
...
//			}
	...
	private void interruptThread() {
		...
		if (crawlpath != null) {
			...
			FragmentationPlugin.writeCrawlPath(context.getSession(), crawlpath, context.getSession().getCrawlPaths().size());
		}
	...
	private boolean inspectNewDom(Eventable event, StateVertex newState) {
		...
		if (isNewState) {
			...
			if (event.getId() == -1) {
				...
				//TODO: Check if this is working properly. Some events are missing in crawlpaths if they are not fixed in the above code
				if(!fixed) {
					LOG.info("Crawlpath could not be fixed with graph. Using the removed eventable {}", removed.getId());
